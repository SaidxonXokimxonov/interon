...
export default function CurrentResult() {
  ...
  useEffect(() => {
    if (!currentResult) navigate("/user/tests");
  });
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-100 p-4">
        <div className="text-center">
          ...
          <p className="text-gray-500 mt-2">
            Please wait while we fetch your test details
          </p>
        </div>
      </div>
    );
  ...
  const scorePercentage = Math.round(
    (currentResult.score / currentResult.totalQuestion) * 100
  );
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 py-12 px-4 sm:px-6">
      <div className="max-w-4xl mx-auto">
              ...
              <div className="mt-4 md:mt-0">
                <div className="inline-flex items-center px-4 py-2 bg-white bg-opacity-20 rounded-full">
                  ...
                  <span className="font-medium">
                    {currentResult.totalQuestion} Questions
                  </span>
                </div>
              </div>
            </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
          ...
          <div className="p-6">
            {currentResult.answers?.map((item, idx) => (
              <div
                key={idx}
        ...
        <div className="mt-8 flex flex-col sm:flex-row justify-between gap-4">
          <button
            onClick={() => navigate(`/user/tests/${currentResult.testId}`)}
            className="px-6 py-3 bg-gradient-to-r from-blue-500 to-indigo-600 text-white font-medium rounded-xl shadow-md hover:shadow-lg transition-all duration-300 transform hover:-translate-y-0.5"
          >
            Review Test Again
          </button>
          <button
            onClick={() => navigate("/user/results")}
            className="cursor-pointer px-6 py-3 bg-white border border-gray-300 text-gray-700 font-medium rounded-xl shadow hover:bg-gray-50 transition-all"
          >
            Back to Dashboard
          </button>
        </div>
